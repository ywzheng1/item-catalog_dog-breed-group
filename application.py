...
def fbconnect():
	...
	print(" ---- user token result : %s" % result)
	...
	url = "https://graph.facebook.com/v4.0/me?fields=id%2Cname%2Cemail%2Cpicture&access_token=" + login_token
	h = httplib2.Http()
	result = h.request(url, 'GET')[1]
	...
	# Get user picture
	# url = 'https://graph.facebook.com/v4.0/me/picture?%s&redirect=0&height=200&width=200' % token
	# h = httplib2.Http()
	# result = h.request(url, 'GET')[1]
	# data = json.loads(result)
...
def gconnect():
	...
	print("**** google access token: %s" % access_token)
